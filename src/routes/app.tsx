...
function AppLayout() {
  ...
  const wrongNetwork =
    wallets.length > 0 &&
    wallets.some((w) => parseChainId(w.chainId) !== RITUAL_CHAIN_ID);
...
function WrongNetworkGate({ wallets }: { wallets: ReturnType<typeof useWallets>["wallets"] }) {
  ...
  const switchAll = async () => {
    ...
    try {
      ...
      toast.error(
        "Could not switch automatically. Add Ritual Testnet manually: Chain ID 1979, RPC https://rpc.ritualfoundation.org",
      );
    } finally {
      setSwitching(false);
    }
    ...
    <main className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
      <div className="glass mx-auto mt-16 max-w-lg rounded-2xl p-10 text-center">
        ...
        <p className="mx-auto mt-3 max-w-sm text-sm text-muted-foreground">
          Ritual Agents only runs on{" "}
          <span className="font-medium text-foreground">Ritual Testnet</span> (Chain ID 1979).
          Switch your wallet to continue.
        </p>
        <div className="mt-4 space-y-1 rounded-lg border border-border bg-card/30 px-4 py-3 text-left font-mono text-xs text-muted-foreground">
          <div>
            <span className="text-foreground/60">Chain ID: </span>1979
          </div>
          <div>
            <span className="text-foreground/60">RPC: </span>https://rpc.ritualfoundation.org
          </div>
          <div>
            <span className="text-foreground/60">Symbol: </span>RITUAL
          </div>
          <div>
            <span className="text-foreground/60">Explorer: </span>
